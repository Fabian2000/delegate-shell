...
use token::{Token, SpannedToken, Span, StringPart};
...
impl Lexer {
    ...
    fn scan_operator(&mut self) -> SpannedToken {
        ...
        let token = match c {
            ...
            '-' => {
                if self.peek() == Some('>') { self.advance(); Token::Send }
            ...
            '?' => Token::Question,
            _ => {
                // Unknown character — emit as error for now
                Token::Ident(format!("__unknown_{c}"))
            }
        };
